...
start_freq   = 1600.0    # 시작 주파수           ========= 클수록 날카롭고 높은 소리
end_freq     = 300.0     # 끝 주파수             ========= 시작보다 낮게 설정하면 점점 낮아지는 느낌
volume       = 0.9       # 전체 음량 (0 ~ 1)
decay_factor = 6         # 감쇠 강도 (볼륨 줄어드는 속도) == 숫자가 클수록 소리가 빨리 줄어든다.
vibrate_rate = 50        # 진동 추가 주파수 (Hz) ========== 기계적인 "위이잉" 느낌 클수록 강함
noise_level  = 0.03      # 기계적 노이즈 정도 ============= 약간의 노이즈(지직지직한 잡음) 0.05는 가벼운 잡음 포함
